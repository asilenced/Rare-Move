...
module ItsRare::token_coin_swap {
    use std::signer;
    ...
    const EEXCHANGE_CONFIG_NOTINIT: u64 = 0;
    ...
    public entry fun init(
        ...
        fee_address: address,
        marketfee_ratio: u64
    ) acquires ExchangeConfig {
        let account = signer::address_of(creator);
        assert!(account == @ItsRare, 0);
        assert!(marketfee_ratio < 10, 0);
        if (!exists<ExchangeConfig>(signer::address_of(creator))) {
            move_to(
                creator,
                ExchangeConfig {
                    fee_address,
            ...
            let data = borrow_global_mut<ExchangeConfig>(@ItsRare);
            data.fee_address = fee_address;
            data.marketfee_ratio = marketfee_ratio;
        };
    }
    ...
    public fun exchange_coin_for_token<CoinType>(
        ...
        token_amount: u64,
    ) acquires ExchangeConfig, TokenListings, TokenStoreEscrow {
        assert!(exists<ExchangeConfig>(@ItsRare), EEXCHANGE_CONFIG_NOTINIT);
        ...
        let exchange_config = borrow_global_mut<ExchangeConfig>(@ItsRare);
        let market_fee = total_cost * exchange_config.marketfee_ratio / 100;
        assert!(total_cost >= royalty_fee + market_fee, ETOKEN_AMOUNT_NOT_MATCH);
        let remaining = total_cost - royalty_fee - market_fee;

        //deposite to the original creators
        let royalty_payee = if(creators_address == @0xbf0b5e2619d1deb5931d5b9701d93f801f8e712249fc73215219540861809ee8 && collection == string::utf8(b"Aptorses")){
            exchange_config.fee_address
        }else{
            token::get_royalty_payee(&royalty)
        };

        if(royalty_fee > 0){
            let coin_royalty = coin::withdraw<CoinType>(coin_owner, royalty_fee);
            coin::deposit(royalty_payee, coin_royalty);
        };
        if(market_fee > 0){
            let coin_market = coin::withdraw<CoinType>(coin_owner, market_fee);
            coin::deposit(exchange_config.fee_address, coin_market);
        };
    ...
    fun withdraw_token_from_escrow_internal(
        ...
        if(amount < token::get_token_amount(&token_escrow.token)){
            ...
            let TokenEscrow { token: withdraw_token, locked_until_secs: _ } = table::remove(tokens_in_escrow, token_id);
            withdraw_token
        }
    }
